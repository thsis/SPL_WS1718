...
conf_mat_true_rf=0#Matrix, die später die Vergleichswerte zwischen Modell und Datensatz beinhaltet (true positives).
conf_mat_sum_rf=0#Matrix, die später die Vergleichswerte zwischen Modell und Datensatz beinhaltet (all values).
...
for (i in 1:30){#30x Modell mit Zufallszahlen bilden:
  ...
  conf_mat_true_rf=sum(conf_mat_true_rf,conf_mat_rf[1,1],conf_mat_rf[2,2],na.rm=T)#Anzahl der Übereinstimmungen aus der Tabelle übernehmen --> wird mit jedem Schleifendurchlauf um das weitere Modellergebnis ergänzt
  conf_mat_sum_rf=sum(conf_mat_sum_rf+sum(conf_mat_rf,na.rm=T),na.rm=T)#Gesamtsumme der Werte übernehmen --> wird mit jedem Schleifendurchlauf um das weitere Modellergebnis ergänzt
}
...
korr #-->Korrektheitsrate (Fläche unter der Kurve: ~0.8
